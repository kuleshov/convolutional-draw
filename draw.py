...
e=tf.random_normal((batch_size,A,B,n_chan_z), mean=0, stddev=1) # Qsampler noise
lstm_enc = ConvLSTMCell(n_chan_he, filter_size=[3,3], name='encoder') # encoder Op
lstm_dec = ConvLSTMCell(n_chan_hd, filter_size=[3,3], name='decoder') # decoder Op
...
# initial states
h_dec_prev=tf.zeros((batch_size,A,B,n_chan_hd))
h_enc_prev=tf.zeros((batch_size,A,B,n_chan_he))
enc_state=lstm_enc.zero_state(batch_size, A, B)
dec_state=lstm_dec.zero_state(batch_size, A, B)
...
for t in range(T):
    ...
    h_enc, enc_state = encode(enc_state,tf.concat([x,epsilon,h_enc_prev,h_dec_prev], 3))
    z, mus[t], logsigmas[t], sigmas[t] = sampleQ(h_enc)
    h_dec, dec_state = decode(dec_state,tf.concat([z,r_prev],3)) # !
